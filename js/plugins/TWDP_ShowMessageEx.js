 ...
 * ※插件用法※
 * --------------------
 * 显示文章中，显示文章里面使用以下控制符号
 * 
 * #{NEXT} 进入下一个事件
 * 直到下一个显示文章内容。会和下一个显示文章内容合并
 * 
 * #{F:编号} 此对话框转换为Fuki对话框
 * 编号：正数为地图事件ID
 * 0为本事件
 * -1为主角
 * -2之后为队伍成员
 * 如果该行走图不是标准高度，可以#{F:编号,高度}来调整
 *
 * #{P:文件名} 显示头像
 * 当你没选头像时，文件名就是文件名。
 * 如果你选了头像，那么文件名就是“头像名_头像编号_文件名
 * 例如，Package1的第8个头像，中文件名是N，选了头像，所以最终查找的是"Package1_8_n.png"。
